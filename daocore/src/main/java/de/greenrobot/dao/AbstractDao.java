...
public abstract class AbstractDao<T, K> {
    ...
    private long executeInsert(T entity, SQLiteStatement stmt) {
        ...
        //
        /**
         * isDbLockedByCurrentThread这个方法可以用于当前线程是否持有了数据库链接
         * 当db开启了事务,没结束这个时间段,就等于持有了数据库链接锁
         */
        if (db.isDbLockedByCurrentThread()) {
            synchronized (stmt) {
                bindValues(stmt, entity);
    ...
    private void loadAllUnlockOnWindowBounds(Cursor cursor, CursorWindow window, List<T> list) {
        ...
        for (int row = 0; ; row++) {
            ...
            if (row >= windowEnd) {
                //
                /**
                 * 猜测一下cursor.moveToNext()抽出一个moveToNextUnlocked
                 * 是因为在row>=windowEnd的情况下moveToNext()可能会调用
                 * fillWindow这个方法,如果一直在同步代码块中调用的话,在这个
                 * 线程全部弄完之前,别的线程将一直获取不到db connection
                 * 造成死锁,db connection资源将得到浪费,在释放锁的一瞬间,别的线程可能
                 * 抢到,交替执行moveToNext(),等于交替使用db connection资源
                 * 例如A线程从.db文件中拉数据进入CursorWindows的时候
                 * B线程在使用Cursor从CursorWindows缓存中读取的数据,当B线程用完
                 * 缓存中的数据的时候,需要从.db文件中通过db connection拉的时候
                 * 释放锁,A在用缓存中的数据执行逻辑,交替执行
                 */
                ...
                if(window.getStartPosition() != 0) {
                    System.out.println(temp == window);
                }
            } else {
